...
| :--- | :--- | :--- |
| **Interactive Mode** | Runs in a continuous loop, prompting the user for input and executing commands directly (similar to **Bash** or **Zsh**). | `./tash` |
| **Batch Mode** | Accepts a single file argument containing a list of commands, executes them sequentially, and then exits. Ideal for **automation** and **scripting**. | `./tash batch.txt` |
...
| :--- | :--- | :--- |
| `exit` | Terminates the shell process. | Must be called **without** arguments. |
| `cd` | Changes the current working directory. | Requires **exactly one** argument (the target path). |
| `path` | Manages the list of directories where the shell searches for external commands. | Accepts **zero or more** arguments, completely overwriting the existing search path. |
***
## 🛠️ Technology Stack
